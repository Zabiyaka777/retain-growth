...
export const handler: Handler = async (event) => {
  ...
  if (!leadId || notes === undefined) {
    return jsonResponse(400, { error: "leadId і notes обов'язкові" });
  }
  ...
  const { error: updateError } = await supabase
    .from("leads")
    .update({ manager_notes: notes.trim() === "" ? null : notes })
    .eq("id", leadId)
    .eq("org_id", profile.org_id as string);

  if (updateError) {
    console.error("save-lead-notes: update failed", updateError);
    return jsonResponse(500, { error: "Не вдалося зберегти нотатки" });
  }
